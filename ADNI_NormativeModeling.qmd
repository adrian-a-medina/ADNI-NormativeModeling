...
base_path <- "~/GitHub/ADNI-NormativeModeling/data_files/"
setwd(base_path)
...
ggpredict(s_1, c(terms = "age", "phenotype", "sex")) |>
  ...
  labs(title = "Average Cortical Thickness as a Function of Phenotype, Adjusted for Age and Sex",
       ...
       caption = "Note: 'phenotype' factors, 'HC' = Healthy Control; 'MCI' = Mild Cognitive Impairment; & 'AD' = Alzheimer's Dementia.")
...
Conduct pairwise statistical comparisons between controls and Alzheimer's dementia patients on cortical thickness measures. Perform t-tests on each brain region, followed by FDR corrections to adjust for multiple comparisons, organizing the results into a clean data frame with significance levels marked.
...
ggplot(combined_stats_sig, aes(x = reorder(ROI, statistic), y = statistic, color = Comparison)) +
  ...
  labs(
    ...
    y = "T-Statistic Value",
    color = "Comparison",
    caption = "Note: 'HC' = Healthy Control; 'MCI' = Mild Cognitive Impairment; & 'AD' = Alzheimer's Dementia") +
  theme_minimal() +
  theme(
    legend.position = "top",
...
# Basic regression analysis with phenotype as predictor and total outlier score as outcome
...
# Adjusted regression analysis with phenotype as predictor and total outlier score as outcome with age and sex covariates
s2<-lm(total_outlier_score ~ phenotype + age + sex, data = df)
anova(s2)
tab_model(s2, show.ci = 0.95, title = "Adjusted Linear Model: Total Outlier Score as a Function of Phenotype, Adjusted for Age and Sex")

# Plot multiple regression model
ggpredict(s2, c(terms = "age", "phenotype", "sex")) |>
  plot() +
  labs(title = "Total Outlier Score as a Function of Phenotype, Adjusted for Age and Sex",
       x = "Age",
       y = "Total Outlier Score",
       caption = "Note: 'phenotype' factors, 'MCI' = Mild Cognitive Impairment; & 'AD' = Alzheimer's Dementia")
...
shapiro.test(df$total_outlier_score) # determines if a sample of data is normally distributed

```

```{r Counting Outliers in Groups, message=FALSE, warning=FALSE}
# --------- MCI Sample ---------
# Maximum number of outliers
df %>% filter(phenotype == "MCI") %>%
  select(contains("_bin")) %>%
  colSums() %>%
  max()

# Maximum number of regions with outliers 
z <- df %>% filter(phenotype == "MCI") %>%
  select(contains("_bin")) %>%
  colSums()
length(which(z > 0))
...
# --------- AD Sample ---------
# Maximum number of outliers
roi_with_most_outliers <- df %>% filter(phenotype == "AD") %>%
  select(contains("_bin")) %>%
  colSums() %>%
  max()

# Maximum number of regions with outliers 
z <- df %>% filter(phenotype == "AD") %>%
  select(contains("_bin")) %>%
  colSums()
length(which(z > 0))

# Print number of ROIs with at least one outlier
roi_with_at_least_one_outlier <- length(which(z > 0))
roi_with_at_least_one_outlier
rm(z)

# Find mean and range for AD subjects
d <- df %>% filter(phenotype == "AD") 
median(d$total_outlier_score) # Calculate median number of outliers per AD subject
range(d$total_outlier_score) # Calculate range of outliers per AD subject

```

```{r Proportional Analyses, message=FALSE, warning=FALSE}
# Relative outlier count per phenotype group
df.outliers <- as.data.frame(t(sapply(X = df[,grep("_bin", names(df), value = TRUE)], FUN = function(x) {
  # Ensure x is a factor with both levels present
  x_factor <- factor(x, levels = c(0, 1))
  # Create table with phenotype, ensuring all levels of x_factor are used and NA values are ignored
  tbl <- table(x_factor, df$phenotype, useNA = "no")
  # Calculate proportions with respect to the phenotype groups
  prop_table <- prop.table(tbl, margin = 2)
  # Safely check if there are two rows and scale by 100 to convert to percentage, otherwise return NA
  if (nrow(prop_table) >= 2) {
    return(prop_table[2, ] * 100)  # Convert values to percentages
  } else {
    # Return NA or another suitable value if there aren't two rows
    return(rep(NA, length(levels(df$phenotype))))
  }
})))

# Download outlier data frame table as a CSV file
...
write.csv (df.outlierstable, '/Users/adrianmedina/GitHub/ADNI-NormativeModeling/data_files/outlier_table.csv')
rm(df.outlierstable)

# Differences of outliers in each group (using ANOVA)
df.stats <- as.data.frame(sapply(X = df[,grep("_bin", names(df),value = T)], FUN = function(x) summary(aov(x ~ df$phenotype))[[1]][["Pr(>F)"]][1]))
names(df.stats) <- "p_value"
setDT(df.stats, keep.rownames = "ROI")
...
# Extracting F-statistic values
df.f_stats <- as.data.frame(sapply(X = df[,grep("_bin", names(df),value = T)], FUN = function(x) summary(aov(x ~ df$phenotype))[[1]][["F value"]][1]))
names(df.f_stats) <- "f_stat"
setDT(df.f_stats, keep.rownames = "ROI")
...
# Plots for both hemispheres separately, values are 1-p_value, so higher bars are more significant
left.stat.plot <- ggplot(df.stats[grep("L_", df.stats$ROI),], aes(x=ROI, y=1-p_value)) + 
  geom_bar(stat="identity", fill="firebrick") + 
  geom_hline(yintercept = 0.95, linetype = "dashed", color = "black") + # Adding significance line
  theme_cowplot() +
...
cowplot::plot_grid(left.stat.plot, right.stat.plot, labels = "AUTO", nrow = 2)
