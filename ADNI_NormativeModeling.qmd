...
```{r BLR vs HBR Correlation, message=FALSE, warning=FALSE}
...
for(region in brain_regions_list) {
    # Ensure the region is present in both datasets
    if(region %in% names(ADNI_PDC)) {
        # Conduct ANOVA and extract the p-value
        model <- aov(as.formula(paste(region, "~ Group")), data = ADNI_PDC)
