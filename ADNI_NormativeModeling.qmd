...
Prepare the environment for neuroimaging data analysis by setting a CRAN repository, installing and loading necessary packages, and specifying a base directory for data files. Read in any data sets intended for use in these analyses and prepare them for further analysis, including normative modeling. The script is also set to handle the importation of deviation scores once acquired.
:::{.callout-tip}
...
# Read in finalized file
ADNI_274_Merged_Final <- read_csv("ADNI_274_Merged_Final.csv") # <3>

# Read in computed deviation scores (the file you get back after uploading adaptation and testing sets)
ADNI_deviation_scores <- read_csv("ADNI_deviation_scores_BLR.csv") # <4>
...
1. For referernce, this merged data file is composed of two data sets: `NIDP_DX_Dem_NP_MRI.csv` contains clinical/non-imaging variables and MRI manufacturer information, & `ADNI_lh_rh_thickness_subcort_volumes.csv` contains the FreeSurfer brain thickness measures.
...
phenotype_site_counts <- ADNI_274_Merged_Final %>%
...
```{r ADNI BLR vs PDC BLR ANOVA}
# Rename SubjectID variable to Subject_ID
ADNI_deviation_scores <- dplyr::rename(ADNI_deviation_scores, SubjectID = "Subject_ID")

# Merge datasets on SubjectID
ADNI_PDC <- inner_join(ADNI_deviation_scores, PDC_BLR, by = "SubjectID")

# List to store all ANOVA results
...
for (brain_regions in brain_regions_list) {
    ...
    for(region in brain_regions) {
        ...
        # Ensure both variables are present in the dataset
        if(region_hbr %in% names(merged_data) && region_blr %in% names(merged_data)) {
            # Conduct ANOVA and extract the p-value
            model <- aov(merged_data[[region_hbr]] ~ merged_data[[region_blr]])
            anova_results[[region]] <- summary(model)[[1]]["Pr(>F)"]
        }
    }
    # Add subset results to the master list
    all_anova_results <- c(all_anova_results, anova_results)
}

# Convert the list to a data frame for easy viewing
anova_df <- data.frame(
    Region = names(all_anova_results),
    P_Value = sapply(all_anova_results, function(x) x),
    row.names = NULL
)

# Apply the FDR correction to the p-values
anova_df$Adjusted_P_Value <- p.adjust(anova_df$P_Value, method = "fdr")
